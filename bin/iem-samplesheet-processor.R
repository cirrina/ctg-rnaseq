...
##    v1.2
## ========================================== ##
## v1.2 is develoed alongside Uroscan pipeline
...
# ================================================================
all.lines <- suppressMessages(scan(file = opt$sample_sheet, what = "character", sep="\n", nmax = 250))
# str(all.lines)
...
# Assay
# Index Adapters
cat(" ... [Rscript]: reading [Header] section")
header_df <-  read.delim(file=opt$sample_sheet, as.is=F,
                  header = FALSE, sep = ",",
                  skip = iem.index["Header"]-1, nrows = iem.index["Reads"]-iem.index["Header"]-1)

# print(header_df$V1)
if(any(duplicated(header_df$V1))) stop("IEM Sample sheet [Header] contains duplicated entries: ",header_df$V1[duplicated(header_df$V1)])
...
# ====================================================
cat("\n ... [Rscript]: reading [Settings] section")
...
# # ====================================================
# #  Check and set 'SharedFlowCell'
# # ====================================================
#   # If exists and if in correct format
#   # Shall define if project is pooled with other, unrelated projects, on the flowcell
#
# SharedFlowCell_values <- iem_df[iem_df$parameter == 'SharedFlowCell',]$value
#
# # str(data_df)
# if('SharedFlowCell' %in% rownames(header_df)){
#   SharedFlowCell <- header_df['SharedFlowCell','V2']
#   if(is.na(SharedFlowCell))  checklist_flags$SharedFlowCell <- "Warning: 'SharedFlowCell' in [Header] section is NA. Set to approriate flag (true / false)."
#   if(!all(SharedFlowCell %in% SharedFlowCell_values)){
#     checklist_flags$SharedFlowCell <- paste("Warning: 'SharedFlowCell' value in [Header] section is not among allowed values:  ", paste(SharedFlowCell_values, collapse = ", "))
#     checklist_flags$samplesheet_accepted <- FALSE
#   }else{
#     data_df$SharedFlowCell <- SharedFlowCell # Create 'Species' column. Force/overwrite if already present
#     checklist_flags$SharedFlowCell <- SharedFlowCell
#   }
# }else{
#   checklist_flags$SharedFlowCell <- "Warning: 'SharedFlowCell' is not defined. You must indicate if this is a SharedFlowCell run or not using 'SharedFlowCell' in [Header] section."
#   checklist_flags$samplesheet_accepted <- FALSE
#
# }
...
cat("\n ... [Rscript]: ... SampleSheet-demux written to file: ", out.file.name)
...
# ========================================================
 ...
 out.file.name <- opt$output_ctg_sheet
  ...
  cat("\n ... [Rscript]: ... SampleSheet-ctg written to file: ", out.file.name)
...
# ========================================================
  ...
  cat("\n ... [Rscript]: ... log written to file: ", out.file.name, "\n")
